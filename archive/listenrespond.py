...
def recognize_speech_vosk():
    model = Model("/home/msutt/vosk_models/vosk-model-small-en-us-0.15")  # Update the path to the VOSK model
    recognizer = KaldiRecognizer(model, 16000)

    input_device = 'default'  # Specify the device identifier

    try:
        with sd.RawInputStream(samplerate=16000, blocksize=8000, dtype='int16', channels=1, device=input_device) as stream:
